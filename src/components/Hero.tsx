        ...
        >
          ...
          >
            {badges.map((badge, index) => (
              <div
                key={index}
                className="flex items-center gap-2 bg-white/10 backdrop-blur-sm px-4 py-2.5 sm:py-2 rounded-full border border-white/20 w-full sm:w-auto max-w-xs sm:max-w-none"
              >
                <badge.icon className="h-5 w-5 text-golden-yellow flex-shrink-0" />
                <span className="text-sm font-medium text-center leading-tight">{badge.label}</span>
              </div>
            ))}
          </motion.div>
        </motion.div>
